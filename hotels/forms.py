...
class RoomForm(forms.ModelForm):
    ...
    class Meta:
        model = Room
        fields = ('hotel', 'room_type', 'room_Name', 'max_adults',
                  'room_Price', 'total_Rooms', 'room_details', 'room_photo',)

        help_texts = {
            'hotel': _('Select hotel for this room'),
            'room_type': _('Select the type of this room'),
            'room_Name': _('Enter a custom name for this room'),
            'max_adults': _('Enter max number of guests in this room'),
    ...
    def __init__(self, user, *args, **kwargs):
        super(RoomForm, self).__init__(*args, **kwargs)
        self.fields['hotel'].queryset = Hotels.objects.filter(
            contact_person=user)
...
class HotelPackagesForm(forms.ModelForm):
    ...
    class Meta:
        model = HotelPackages
        fields = ('hotel', 'package_Price', 'duration',
...
class ItinireryForm(forms.ModelForm):
    ...
    class Meta:
        model = Itinirery
        fields = ('title', 'package', 'description',)
        ...
        def __init__(self, user, *args, **kwargs):
            ...
            self.fields['package'].queryset = Packages.objects.filter(
                user=user)
...
class ConferenceRoomForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('hotel', 'room_Name', 'room_Capacity', 'room_Price',
                  'room_discount',  'room_details',)
